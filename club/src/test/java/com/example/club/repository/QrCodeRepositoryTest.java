...
class QrCodeRepositoryTest extends AbstractIntegrationTest {
    ...
    @Test
    void testFindByUuidValueAndIsActiveTrue() {
        Participant participant = createParticipant("Иван Иванов");
        UUID uuid = UUID.randomUUID();

        QrCode qrCode = new QrCode();
        qrCode.setUuidValue(uuid);
        qrCode.setParticipant(participant);
        qrCode.setIsActive(true);
        qrCode.setCreatedAt(LocalDateTime.now());
        qrCodeRepository.save(qrCode);

        var found = qrCodeRepository.findByUuidValueAndIsActiveTrue(uuid);

        assertTrue(found.isPresent());
        assertEquals(uuid, found.get().getUuidValue());
        assertTrue(found.get().getIsActive());
    }

    @Test
    void testFindByUuidValueAndIsActiveTrueNotFound() {
        var found = qrCodeRepository.findByUuidValueAndIsActiveTrue(UUID.randomUUID());
    ...
    @Test
    void testFindByUuidValueAndIsActiveTrueOrThrow() {
        Participant participant = createParticipant("Иван Иванов");
        UUID uuid = UUID.randomUUID();
        ...
        QrCode found = qrCodeRepository.findByUuidValueAndIsActiveTrueOrThrow(uuid);

        assertNotNull(found);
        assertEquals(uuid, found.getUuidValue());
    }

    @Test
    void testFindByUuidValueAndIsActiveTrueOrThrowNotFound() {
        Exception exception = assertThrows(RuntimeException.class, () -> {
            qrCodeRepository.findByUuidValueAndIsActiveTrueOrThrow(UUID.randomUUID());
        });
        assertEquals("QR-код не найден или уже использован", exception.getMessage());
    }
